            ...
            if channel.isMember {
                for message in channel.messages.reverse() {
                    if let messageText = message.text {
                        ...
                        messageLabel.frame.size = messageLabel.attributedStringValue.boundingRectWithSize(NSSize(width: messageViewSize.width, height: 300.0), options: NSStringDrawingOptions.UsesLineFragmentOrigin).size
                        messageLabel.frame.size.width += 100.0
